...
$qualified_parties = [];
foreach ($national_results as &$res) { // Use reference to modify array directly
    $percentage = ($national_total > 0) ? ($res['total_votes'] / $national_total) * 100 : 0;
    $res['percentage'] = $percentage; // Add percentage to the array
    if ($percentage >= NATIONAL_THRESHOLD_PERCENT) {
        $qualified_parties[] = $res['party_id'];
    }
}
unset($res); // Unset the reference after the loop
...
<body class="bg-light">
    ...
    <div class="container mt-4">
        <div class="alert alert-info">
            <strong>Moroccan Election Simulation:</strong> National results with <?php echo NATIONAL_THRESHOLD_PERCENT; ?>% threshold and proportional seat allocation (Largest Remainder Method). Total seats: <?php echo $total_seats_national; ?>.
        </div>
